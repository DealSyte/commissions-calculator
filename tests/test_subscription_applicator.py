...
class TestAdvanceFeeApplication:
    ...
    def test_no_implied_creates_no_advance_fees(self, applicator):
        ...
        ctx = self._make_context(
            implied_after_credit=0, payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("0"))]
        )
    ...
    def test_implied_fully_covers_single_payment(self, applicator):
        ...
        ctx = self._make_context(
            implied_after_credit=5000, payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("0"))]
        )
        result = applicator.apply(ctx)
    ...
    def test_implied_partially_covers_payment(self, applicator):
        ...
        ctx = self._make_context(
            implied_after_credit=3000, payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("0"))]
        )
        result = applicator.apply(ctx)
    ...
    def test_implied_exceeds_total_owed(self, applicator):
        ...
        ctx = self._make_context(
            implied_after_credit=10000, payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("0"))]
        )
        result = applicator.apply(ctx)
    ...
    def test_partially_paid_payment_considered(self, applicator):
        ...
        ctx = self._make_context(
            implied_after_credit=3000, payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("2000"))]
        )
        result = applicator.apply(ctx)
    ...
    def test_no_payments_is_fully_prepaid(self, applicator):
        ...
        ctx = self._make_context(implied_after_credit=5000, payments=[])
        result = applicator.apply(ctx)
    ...
    def test_payg_skips_subscription_entirely(self, applicator):
        ...
        ctx = self._make_context(
            ...
            payments=[FuturePayment("p1", "2026-06-01", Decimal("5000"), Decimal("0"))],
            is_payg=True,
        )
        result = applicator.apply(ctx)
    ...
    def _make_context(self, implied_after_credit: float, payments: list, is_payg: bool = False) -> ProcessingContext:
        deal = Deal(
            name="Test",
        ...
        ctx.credit = CreditApplication(implied_after_credit=Decimal(str(implied_after_credit)))
        return ctx
