...
class TestCreditGeneration:
    ...
    def test_debt_collected_becomes_credit(self, applicator):
        ...
        ctx = _make_credit_context(
            current_credit=0, debt_collected=5000, implied_total=3000, is_in_commissions_mode=False
        )
    ...
    def test_existing_credit_plus_new_credit(self, applicator):
        ...
        ctx = _make_credit_context(
            current_credit=2000, debt_collected=3000, implied_total=4000, is_in_commissions_mode=False
        )
        result = applicator.apply(ctx)
    ...
    def test_payg_generates_no_credit(self, applicator):
        ...
        ctx = _make_credit_context(current_credit=0, debt_collected=5000, implied_total=3000, is_payg=True)
        result = applicator.apply(ctx)
...
class TestCreditApplication:
    ...
    def test_credit_absorbs_implied_fully(self, applicator):
        ...
        ctx = _make_credit_context(
            current_credit=5000, debt_collected=0, implied_total=3000, is_in_commissions_mode=False
        )
        result = applicator.apply(ctx)
    ...
    def test_credit_partially_absorbs_implied(self, applicator):
        ...
        ctx = _make_credit_context(
            current_credit=2000, debt_collected=0, implied_total=5000, is_in_commissions_mode=False
        )
        result = applicator.apply(ctx)
    ...
    def test_no_credit_leaves_implied_unchanged(self, applicator):
        ...
        ctx = _make_credit_context(current_credit=0, debt_collected=0, implied_total=5000, is_in_commissions_mode=False)
        result = applicator.apply(ctx)
    ...
    def test_commissions_mode_skips_credit_usage(self, applicator):
        ...
        ctx = _make_credit_context(
            current_credit=5000, debt_collected=0, implied_total=3000, is_in_commissions_mode=True
        )
        result = applicator.apply(ctx)
...
def _make_credit_context(
    ...
    contract = Contract(
        rate_type="fixed", fixed_rate=Decimal("0.05"), accumulated_success_fees=Decimal("0"), is_pay_as_you_go=is_payg
    )
    state = ContractState(
