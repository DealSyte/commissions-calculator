...
class TestCostCapEnforcement:
    ...
    def test_invalid_cap_type_ignored(self, enforcer):
        ...
        ctx = self._make_context(
            commissions=5000,
            advance_fees=0,
            cost_cap_type='invalid',
            cost_cap_amount=1000,
            total_paid=0
        )
        result = enforcer.apply(ctx)
    ...
    def _make_context(
        ...
        ctx.subscription = SubscriptionApplication(
            advance_fees_created=Decimal(str(advance_fees))
        )
        ctx.commission = CommissionCalculation(
            finalis_commissions_before_cap=Decimal(str(commissions)),
            finalis_commissions=Decimal(str(commissions))
        )
        return ctx
